<x-app-layout>
    ...
    <!-- PESANAN -->
      ...
      <div class="bg-white rounded-xl border p-6">
         ...
         <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <x-detail label="Frame"
               :value="$RmPemeriksaan->pesanan->frame->merk.' - '.$RmPemeriksaan->pesanan->frame->kode_frame" />
            <x-detail label="Lensa"
               :value="$RmPemeriksaan->pesanan->lensa->nama_lensa" />
            <x-detail label="Biaya"
               :value="'Rp '.number_format($RmPemeriksaan->pesanan->biaya_kacamata,0,',','.')" />

   <p class="text-sm text-gray-500 mb-1">Status</p>

   @php
      $status = $RmPemeriksaan->pesanan->status;
   @endphp
   ...
   @endif
            ...
            <x-detail label="Tanggal Dipesan"
               :value="$RmPemeriksaan->pesanan->tanggal_dipesan" />
            <x-detail label="Tanggal Pengambilan"
               :value="$RmPemeriksaan->pesanan->tanggal_pengambilan ?? '-'" />
         </div>
      </div>
      @endif
            ...
            <table class="w-full text-left border border-gray-200 rounded-lg overflow-hidden">
               <tbody class="divide-y divide-gray-200">
                  ...
                  <tr>
                     ...
                     <td class="px-4 py-1 font-semibold text-gray-900"> {{ $RmPemeriksaan->diagnosa ?? '-' }} </td>
                  </tr>
               </tbody>
            </table>
         </div>
               ...
               <table class="w-full border border-gray-200 text-sm">
                  ...
                  <tbody class="text-gray-800">
                     <tr>
                        ...
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->od_sferis ?? '-' }}</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->os_sferis ?? '-' }}</td>
                     </tr>
                     <tr>
                        <td class="border px-4 py-2">Silindris</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->od_silindris ?? '-' }}</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->os_silindris ?? '-' }}</td>
                     </tr>
                     <tr>
                        <td class="border px-4 py-2">Axis</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->od_axis ?? '-' }}</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->os_axis ?? '-' }}</td>
                     </tr>
                     <tr>
                        <td class="border px-4 py-2">Add</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->od_add_lensa ?? '-' }}</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->os_add_lensa ?? '-' }}</td>
                     </tr>
                     <tr>
                        <td class="border px-4 py-2">PD</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->pd_od ?? '-' }}</td>
                        <td class="border px-4 py-2 text-center">{{ $RmPemeriksaan->resep->pd_os ?? '-' }}</td>
                     </tr>
                  </tbody>
               </table>
            </div>
      ...
      <!-- PENGAMBILAN -->
      @if($RmPemeriksaan->pesanan && strtolower($RmPemeriksaan->pesanan->status) == 'diambil' && $RmPemeriksaan->pesanan->pengambilan)
      <div class="bg-white rounded-xl border p-6">
