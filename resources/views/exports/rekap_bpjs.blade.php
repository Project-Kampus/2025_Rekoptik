...
<table border="1" style="border-collapse: collapse; width: 100%;">
    <thead>
        <tr>
            ...
            <th colspan="1" rowspan="2">No.Kartu BPJS</th>
    ...
    <tbody>
        @foreach ($rekamMedis as $index => $rm)
            <tr>
                ...
                <td>{{ $rm->pembayarans->where('metode', 'bpjs')->sum('jumlah') ?? 0 }}</td>
                <td>{{ ($rm->biaya_kacamata ?? 0) - ($rm->pembayarans->where('metode', 'bpjs')->sum('jumlah') ?? 0) }}
                </td>
            </tr>
        @endforeach
    </tbody>
